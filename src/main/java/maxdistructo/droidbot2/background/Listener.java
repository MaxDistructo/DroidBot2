...
public class Listener {
    ...
    public void onMessageReceivedEvent(MessageReceivedEvent event) throws RateLimitException, DiscordException, MissingPermissionsException { // This method is NOT called because it doesn't have the @EventSubscriber annotation
        try {
            ...
            if(playerFun){
                LOGGER.info("PlayerFun Commands Enabled");
                client.registerListener(pfListener);
            }
            else{
                LOGGER.warn("PlayerFun Commands Not Enabled");
            }
            if(casino){
                LOGGER.info("Casino Commands Enabled");
                client.registerListener(casinoListener);
            }
            else{
                LOGGER.warn("Casino Commands Not Enabled");
            }
            if(admin){
                LOGGER.info("Admin Commands Enabled");
                client.registerListener(adminListener);
            }
            else{
                LOGGER.warn("Admin Commands Not Enabled");
            }
